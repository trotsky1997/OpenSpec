...
export class UpdateCommand {
	...
	async execute(projectPath: string): Promise<void> {
		...
		const desiredWorkflows = profileWorkflows.filter(
		...
		if (!this.force && toolsToUpdateSet.size === 0) {
			...
			this.displayExtraWorkflowsNote(
				resolvedProjectPath,
				configuredTools,
				desiredWorkflows,
			);
		...
		const commandContents = shouldGenerateCommands
			? getCommandContents(desiredWorkflows, COMMAND_NAMESPACES)
			: [];
			...
			try {
				...
				if (shouldGenerateCommands) {
					...
					if (adapter) {
						...
						removedDeselectedCommandCount +=
							await this.removeUnselectedCommandFiles(
								resolvedProjectPath,
								toolId,
								desiredWorkflows,
							);
		...
		if (updatedTools.length > 0) {
			console.log(
				chalk.green(
					`✓ Updated: ${updatedTools.join(", ")} (v${OPENSPEC_VERSION})`,
				),
			);
		}
		...
		if (removedCommandCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${removedCommandCount} command files (delivery: skills)`,
				),
			);
		}
		if (removedSkillCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${removedSkillCount} skill directories (delivery: commands)`,
				),
			);
		}
		if (removedDeselectedCommandCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${removedDeselectedCommandCount} command files (deselected workflows)`,
				),
			);
		}
		if (removedDeselectedSkillCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${removedDeselectedSkillCount} skill directories (deselected workflows)`,
				),
			);
		}
		...
		if (newlyConfiguredTools.length > 0) {
			...
			console.log("  /opsx:new or /ssx:new       Start a new change");
			console.log(
				"  /opsx:continue or /ssx:continue  Create the next artifact",
			);
			console.log("  /opsx:apply or /ssx:apply     Implement tasks");
			console.log();
			console.log(
		...
		const configuredAndNewTools = [
			...new Set([...configuredTools, ...newlyConfiguredTools]),
		];
	...
	private displayUpToDateMessage(toolStatuses: ToolVersionStatus[]): void {
		...
		console.log(
			chalk.green(
				`✓ All ${toolStatuses.length} tool(s) up to date (v${OPENSPEC_VERSION})`,
			),
		);
	...
	private displayExtraWorkflowsNote(
		...
		const installedWorkflows = scanInstalledWorkflows(
			projectPath,
			configuredTools,
		);
		...
		if (extraWorkflows.length > 0) {
			console.log(
				chalk.dim(
					`Note: ${extraWorkflows.length} extra workflows not in profile (use \`openspec config profile\` to manage)`,
				),
			);
		}
	}
	...
	private async handleLegacyCleanup(
		...
		if (this.force) {
			...
			return this.upgradeLegacyTools(
				projectPath,
				detection,
				canPrompt,
				desiredWorkflows,
	...
	private async upgradeLegacyTools(
		...
		if (this.force || !canPrompt) {
			...
			const { searchableMultiSelect } = await import(
				"../prompts/searchable-multi-select.js"
			);
