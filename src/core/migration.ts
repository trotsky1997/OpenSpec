	...
	for (const tool of tools) {
		...
		for (const workflowId of ALL_WORKFLOWS) {
			for (const namespace of COMMAND_NAMESPACES) {
				const commandPath = adapter.getFilePath(workflowId, namespace);
...
export function migrateIfNeeded(
	...
	console.log(
		`Migrated: custom profile with ${installedWorkflows.length} workflows`,
	);
	console.log(
		"New in this version: /opsx:propose and /ssx:propose. Try 'openspec config profile core' for the streamlined experience.",
	);
}
