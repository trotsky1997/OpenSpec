...
function toKnownWorkflows(workflows: readonly string[]): WorkflowId[] {
	return workflows.filter((workflow): workflow is WorkflowId =>
...
export function toolHasAnyConfiguredCommand(
	...
	for (const commandId of COMMAND_IDS) {
		for (const namespace of COMMAND_NAMESPACES) {
			const cmdPath = adapter.getFilePath(commandId, namespace);
 ...
 */
export function getConfiguredToolsForProfileSync(
	projectPath: string,
): string[] {
