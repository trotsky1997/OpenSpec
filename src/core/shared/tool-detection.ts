 ...
 */
export function extractGeneratedByVersion(
	...
	try {
		...
		//   generatedBy: "0.23.0"
		// ---
		const generatedByMatch = content.match(
			/^\s*generatedBy:\s*["']?([^"'\n]+)["']?\s*$/m,
		);
...
export function getToolVersionStatus(
	...
	const needsUpdate =
		configured &&
		(generatedByVersion === null || generatedByVersion !== currentVersion);
...
export function getConfiguredTools(projectRoot: string): string[] {
	return AI_TOOLS.filter(
		(t) => t.skillsDir && getToolSkillStatus(projectRoot, t.value).configured,
	).map((t) => t.value);
}
