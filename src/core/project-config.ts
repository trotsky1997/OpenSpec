...
import { z } from "zod";
import { STRICT_WORKFLOW_VARIANTS } from "../utils/strict-workflow.js";
...
export const ProjectConfigSchema = z.object({
	...
	// Optional: default strict workflow variant for new changes
	variant: z
		.enum(STRICT_WORKFLOW_VARIANTS)
		.optional()
		.describe('Default change execution variant (e.g., "solidspec")'),
...
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
	...
	try {
		...
		if (raw.variant !== undefined) {
			const variantField = z.enum(STRICT_WORKFLOW_VARIANTS);
			const variantResult = variantField.safeParse(raw.variant);

			if (variantResult.success) {
				config.variant = variantResult.data;
			} else {
				console.warn(
					`Invalid 'variant' field in config (must be one of: ${STRICT_WORKFLOW_VARIANTS.join(", ")})`,
				);
			}
		}
