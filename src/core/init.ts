...
const WORKFLOW_TO_SKILL_DIR: Record<string, string> = {
	...
	inspect: "openspec-inspect-change",
	clarify: "openspec-clarify-change",
...
export class InitCommand {
	...
	async execute(targetPath: string): Promise<void> {
		...
		const selectedToolIds = await this.getSelectedTools(
			toolStates,
			extendMode,
			detectedTools,
			projectPath,
	...
	private async getSelectedTools(
		...
		const sortedChoices = validTools
			.map((toolId) => {
				...
				return {
					...
					preSelected:
						configured || (shouldPreselectDetected && detected && !configured),
				};
			})
			.sort((a, b) => {
		...
		const configuredNames = validTools
			...
			.map(
				(toolId) => AI_TOOLS.find((t) => t.value === toolId)?.name || toolId,
			);

		if (configuredNames.length > 0) {
			console.log(
				`OpenSpec configured: ${configuredNames.join(", ")} (pre-selected)`,
			);
		}
		...
		if (detectedOnlyNames.length > 0) {
			const detectionLabel = shouldPreselectDetected
				...
				: "not pre-selected";
			console.log(
				`Detected tool directories: ${detectedOnlyNames.join(", ")} (${detectionLabel})`,
			);
		}
		...
		const selectedTools = await searchableMultiSelect({
			...
			validate: (selected: string[]) =>
				selected.length > 0 || "Select at least one tool",
		});
	...
	private async generateSkillsAndCommands(
		...
		const profile: Profile =
			this.resolveProfileOverride() ?? globalConfig.profile ?? "core";
	...
	private displaySuccessMessage(
		...
		if (results.createdTools.length > 0) {
			console.log(
				`Created: ${results.createdTools.map((t) => t.name).join(", ")}`,
			);
		}
		if (results.refreshedTools.length > 0) {
			console.log(
				`Refreshed: ${results.refreshedTools.map((t) => t.name).join(", ")}`,
			);
		}
		...
		if (successfulTools.length > 0) {
			const globalConfig = getGlobalConfig();
			const profile: Profile =
				(this.profileOverride as Profile) ?? globalConfig.profile ?? "core";
			const delivery: Delivery = globalConfig.delivery ?? "both";
			const workflows = getProfileWorkflows(profile, globalConfig.workflows);
			const toolDirs = [
		...
		if (results.failedTools.length > 0) {
			console.log(
				chalk.red(
					`Failed: ${results.failedTools.map((f) => `${f.name} (${f.error.message})`).join(", ")}`,
				),
			);
		}
		...
		if (results.commandsSkipped.length > 0) {
			console.log(
				chalk.dim(
					`Commands skipped for: ${results.commandsSkipped.join(", ")} (no adapter)`,
				),
			);
		}
		if (results.removedCommandCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${results.removedCommandCount} command files (delivery: skills)`,
				),
			);
		}
		if (results.removedSkillCount > 0) {
			console.log(
				chalk.dim(
					`Removed: ${results.removedSkillCount} skill directories (delivery: commands)`,
				),
			);
		}
		...
		const activeProfile: Profile =
			(this.profileOverride as Profile) ?? globalCfg.profile ?? "core";
		const activeWorkflows = [
			...getProfileWorkflows(activeProfile, globalCfg.workflows),
		];
		console.log();
