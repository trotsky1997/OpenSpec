	...
	try {
		...
		if (!options.change) {
			...
			if (available.length === 0) {
				...
				console.log(
					"No active changes. Create one with: openspec new change <name>",
				);
				return;
			}
...
export function printStatusText(status: ChangeStatus): void {
	...
	if (status.managedFiles && status.managedFiles.length > 0) {
		console.log("Managed files:");
		for (const filePath of status.managedFiles) {
