...
const { confirmMock, showWelcomeScreenMock, searchableMultiSelectMock } =
	vi.hoisted(() => ({
		confirmMock: vi.fn(),
...
describe("InitCommand", () => {
	...
	beforeEach(async () => {
		...
		configTempDir = path.join(
			os.tmpdir(),
			`openspec-config-init-${Date.now()}`,
		);
	...
	describe("error handling", () => {
		...
		it("should throw error in non-interactive mode without --tools flag and no detected tools", async () => {
			...
			await expect(initCommand.execute(testDir)).rejects.toThrow(
				/No tools detected and no --tools flag/,
			);
		});
	});
...
describe("InitCommand - profile and detection features", () => {
	...
	beforeEach(async () => {
		testDir = path.join(
			os.tmpdir(),
			`openspec-init-profile-test-${Date.now()}`,
		);
		await fs.mkdir(testDir, { recursive: true });
		originalEnv = { ...process.env };
	...
	it("should use --profile flag to override global config", async () => {
		...
		const newChangeSkill = path.join(
			testDir,
			".claude",
			"skills",
			"openspec-new-change",
	...
	it("should auto-cleanup legacy artifacts in non-interactive mode without --force", async () => {
		...
		await fs.writeFile(
			path.join(legacyDir, "opsx-propose.md"),
			"legacy content",
		);
		...
		expect(await fileExists(path.join(legacyDir, "opsx-propose.md"))).toBe(
			false,
		);
	...
	it("should preselect configured tools but not directory-detected tools in extend mode", async () => {
		...
		vi.spyOn(initCommand as any, "canPromptInteractively").mockReturnValue(
			true,
		);
		...
		const [{ choices }] = searchableMultiSelectMock.mock.calls[0] as [
			{
				choices: Array<{
					value: string;
					preSelected?: boolean;
		...
		const githubCopilot = choices.find(
			(choice) => choice.value === "github-copilot",
		);
	...
	it("should migrate commands-only extend mode to custom profile without injecting propose", async () => {
		await fs.mkdir(path.join(testDir, "openspec"), { recursive: true });
		await fs.mkdir(path.join(testDir, ".claude", "commands", "opsx"), {
			recursive: true,
		});
		await fs.writeFile(
		...
		const exploreCommand = path.join(
			testDir,
			".claude",
			"commands",
			"opsx",
		...
		const proposeCommand = path.join(
			testDir,
			".claude",
			"commands",
			"opsx",
	...
	it("should not prompt for confirmation when applying custom profile in interactive init", async () => {
		...
		vi.spyOn(initCommand as any, "getSelectedTools").mockResolvedValue([
			"claude",
		]);
		...
		const logCalls = (
			...
			.map(String);
		expect(
			logCalls.some((entry) => entry.includes("Applying custom profile")),
		).toBe(false);
	});
