...
`,
			...
			it("should parse valid default variant", () => {
				const configDir = path.join(tempDir, "openspec");
				fs.mkdirSync(configDir, { recursive: true });
				fs.writeFileSync(
					path.join(configDir, "config.yaml"),
			...
			it("should ignore invalid variant values", () => {
				...
				expect(consoleWarnSpy).toHaveBeenCalledWith(
					expect.stringContaining("Invalid 'variant' field"),
				);
			});
...
`,
				...
				expect(consoleWarnSpy).toHaveBeenCalledWith(
					expect.stringContaining(
						"Rules for 'specs' must be an array of strings",
					),
				);
			});
...
`,
				...
				expect(consoleWarnSpy).toHaveBeenCalledWith(
					expect.stringContaining(
						"Some rules for 'proposal' are empty strings",
					),
				);
			});
...
`,
	...
	describe("validateConfigRules", () => {
		...
		it("should warn about unknown artifact IDs", () => {
			...
			expect(warnings[0]).toContain(
				'Valid IDs for schema "spec-driven": design, proposal, specs, tasks',
			);
			expect(warnings[1]).toContain(
				'Unknown artifact ID in rules: "documentation"',
			);
		});
	...
	describe("suggestSchemas", () => {
		...
		it("should handle case when no project-local schemas exist", () => {
			const builtInOnly = [{ name: "spec-driven", isBuiltIn: true }];
