...
function writeCommand(projectDir: string, workflowId: string): void {
	...
	if (!adapter)
		throw new Error("Claude adapter unavailable in test environment");
	for (const namespace of COMMAND_NAMESPACES) {
		const cmdPath = adapter.getFilePath(workflowId, namespace);
		const fullPath = path.isAbsolute(cmdPath)
			? cmdPath
			: path.join(projectDir, cmdPath);
		fs.mkdirSync(path.dirname(fullPath), { recursive: true });
		fs.writeFileSync(fullPath, `# ${namespace}:${workflowId}\n`);
	}
}
...
describe("profile sync drift detection", () => {
	...
	beforeEach(() => {
		tempDir = path.join(
			os.tmpdir(),
			`openspec-profile-sync-drift-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
