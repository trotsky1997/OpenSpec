...
describe("command-generation/adapters", () => {
	...
	describe("claudeAdapter", () => {
		...
		it("should generate correct file path for different command IDs", () => {
			expect(claudeAdapter.getFilePath("new")).toBe(
				path.join(".claude", "commands", "opsx", "new.md"),
			);
			expect(claudeAdapter.getFilePath("bulk-archive")).toBe(
				path.join(".claude", "commands", "opsx", "bulk-archive.md"),
			);
		});

		it("should generate ssx alias paths for nested-directory adapters", () => {
			expect(claudeAdapter.getFilePath("explore", "ssx")).toBe(
				path.join(".claude", "commands", "ssx", "explore.md"),
			);
		});
	...
	describe("cursorAdapter", () => {
		...
		it("should generate correct file path with opsx- prefix", () => {
			...
			expect(filePath).toBe(
				path.join(".cursor", "commands", "opsx-explore.md"),
			);
		});

		it("should generate correct file paths for different commands", () => {
			expect(cursorAdapter.getFilePath("new")).toBe(
				path.join(".cursor", "commands", "opsx-new.md"),
			);
			expect(cursorAdapter.getFilePath("bulk-archive")).toBe(
				path.join(".cursor", "commands", "opsx-bulk-archive.md"),
			);
		});

		it("should generate ssx alias paths and frontmatter for hyphen-prefixed adapters", () => {
			const output = cursorAdapter.formatFile({
				...sampleContent,
				namespace: "ssx",
				name: "SSX: Explore",
			});

			expect(cursorAdapter.getFilePath("explore", "ssx")).toBe(
				path.join(".cursor", "commands", "ssx-explore.md"),
			);
			expect(output).toContain("name: /ssx-explore");
			expect(output).toContain("id: ssx-explore");
		});
	...
	describe("windsurfAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".windsurf", "workflows", "opsx-explore.md"),
			);
		});
	...
	describe("amazonQAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".amazonq", "prompts", "opsx-explore.md"),
			);
		});
	...
	describe("antigravityAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".agent", "workflows", "opsx-explore.md"),
			);
		});
	...
	describe("auggieAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".augment", "commands", "opsx-explore.md"),
			);
		});
	...
	describe("bobAdapter", () => {
		...
		it("should generate correct file paths for different commands", () => {
			expect(bobAdapter.getFilePath("new")).toBe(
				path.join(".bob", "commands", "opsx-new.md"),
			);
			expect(bobAdapter.getFilePath("bulk-archive")).toBe(
				path.join(".bob", "commands", "opsx-bulk-archive.md"),
			);
		});
	...
	describe("clineAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".clinerules", "workflows", "opsx-explore.md"),
			);
		});
	...
	describe("codexAdapter", () => {
		...
		it("should respect CODEX_HOME env var", () => {
			...
			try {
				...
				expect(filePath).toBe(
					path.join(
						path.resolve("/custom/codex-home"),
						"prompts",
						"opsx-explore.md",
					),
				);
	...
	describe("codebuddyAdapter", () => {
		...
		it("should generate correct file path with nested opsx folder", () => {
			...
			expect(filePath).toBe(
				path.join(".codebuddy", "commands", "opsx", "explore.md"),
			);
		});
	...
	describe("continueAdapter", () => {
		...
		it("should generate correct file path with .prompt extension", () => {
			...
			expect(filePath).toBe(
				path.join(".continue", "prompts", "opsx-explore.prompt"),
			);
		});
	...
	describe("crushAdapter", () => {
		...
		it("should generate correct file path with nested opsx folder", () => {
			...
			expect(filePath).toBe(
				path.join(".crush", "commands", "opsx", "explore.md"),
			);
		});
	...
	describe("factoryAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".factory", "commands", "opsx-explore.md"),
			);
		});
	...
	describe("geminiAdapter", () => {
		...
		it("should generate correct file path with .toml extension", () => {
			...
			expect(filePath).toBe(
				path.join(".gemini", "commands", "opsx", "explore.toml"),
			);
		});
	...
	describe("githubCopilotAdapter", () => {
		...
		it("should generate correct file path with .prompt.md extension", () => {
			...
			expect(filePath).toBe(
				path.join(".github", "prompts", "opsx-explore.prompt.md"),
			);
		});
	...
	describe("kilocodeAdapter", () => {
		...
		it("should generate correct file path", () => {
			...
			expect(filePath).toBe(
				path.join(".kilocode", "workflows", "opsx-explore.md"),
			);
		});
...
/opsx:apply to implement`,
	...
	describe("qoderAdapter", () => {
		...
		it("should generate correct file path with nested opsx folder", () => {
			...
			expect(filePath).toBe(
				path.join(".qoder", "commands", "opsx", "explore.md"),
			);
		});
	...
	describe("qwenAdapter", () => {
		...
		it("should generate correct file path with .toml extension", () => {
			...
			expect(filePath).toBe(
				path.join(".qwen", "commands", "opsx-explore.toml"),
			);
		});
	...
	describe("piAdapter", () => {
		...
		it("should generate ssx alias paths and hyphenated references for pi prompts", () => {
			const output = piAdapter.formatFile({
				...sampleContent,
				namespace: "ssx",
				body: "Run /ssx:apply to implement. Then /ssx:archive when done.",
			});

			expect(piAdapter.getFilePath("explore", "ssx")).toBe(
				path.join(".pi", "prompts", "ssx-explore.md"),
			);
			expect(output).toContain("/ssx-apply");
			expect(output).toContain("/ssx-archive");
			expect(output).not.toContain("/ssx:apply");
		});

		it("should generate correct file paths for different commands", () => {
			expect(piAdapter.getFilePath("new")).toBe(
				path.join(".pi", "prompts", "opsx-new.md"),
			);
			expect(piAdapter.getFilePath("bulk-archive")).toBe(
				path.join(".pi", "prompts", "opsx-bulk-archive.md"),
			);
		});
	...
	describe("cross-platform path handling", () => {
		it("Claude adapter uses path.join for paths", () => {
			...
			expect(filePath.split(path.sep)).toEqual([
				".claude",
				"commands",
				"opsx",
				"test.md",
		...
		it("All adapters use path.join for paths", () => {
			...
			for (const adapter of adapters) {
				...
				expect(filePath.includes(path.sep) || filePath.includes(".")).toBe(
					true,
				);
			}
		});
