...
`,
		...
		it("should not show --force hint when force is used", async () => {
			...
			const hasForceHint = allCalls.some((call) =>
				call.includes("Use --force"),
			);
...
`;
		...
		it("should cleanup legacy slash command directories with --force", async () => {
			...
			const legacyCommandDir = path.join(
				testDir,
				".claude",
				"commands",
				"openspec",
			...
			const legacyDirExists =
				await FileSystemUtils.directoryExists(legacyCommandDir);
...
`;
	...
	describe("legacy tool upgrade", () => {
		...
		it("should upgrade multiple legacy tools with --force", async () => {
			...
			await fs.mkdir(path.join(testDir, ".claude", "commands", "openspec"), {
				recursive: true,
			});
			await fs.writeFile(
			...
			await fs.mkdir(path.join(testDir, ".cursor", "commands"), {
				recursive: true,
			});
			await fs.writeFile(
			...
			const cursorSkillFile = path.join(
				testDir,
				".cursor",
				"skills",
				"openspec-explore",
		...
		it("should not inject non-profile workflows when upgrading legacy tools", async () => {
			...
			const commandsDir = path.join(testDir, ".claude", "commands", "opsx");
			expect(
				await FileSystemUtils.fileExists(path.join(commandsDir, "explore.md")),
			).toBe(true);
			expect(
				await FileSystemUtils.fileExists(path.join(commandsDir, "propose.md")),
			).toBe(false);
		});
	});
	...
	describe("profile-aware updates", () => {
		...
		it("should remove skills for configured tools without command adapters in commands-only delivery", async () => {
			...
			const { CommandAdapterRegistry } = await import(
				"../../src/core/command-generation/index.js"
			);
			const adapterlessTool = AI_TOOLS.find(
				(tool) => tool.skillsDir && !CommandAdapterRegistry.get(tool.value),
			);
...
`,
		...
		it("should remove workflows outside profile during update sync", async () => {
			...
			await fs.mkdir(path.join(skillsDir, "openspec-new-change"), {
				recursive: true,
			});
			await fs.writeFile(
				path.join(skillsDir, "openspec-new-change", "SKILL.md"),
			...
			const extraCommandFile = path.join(
				testDir,
				".claude",
				"commands",
				"opsx",
	...
	describe("new tool detection", () => {
		it("should detect new tool directories not currently configured", async () => {
			...
			const hasNewToolMessage = calls.some((call) =>
				call.includes(
					"Detected new tool: Cursor. Run 'openspec init' to add it.",
				),
			);
		...
		it("should consolidate multiple new tools into one message", async () => {
			...
			expect(consolidatedCalls[0]).toContain(
				"Run 'openspec init' to add them.",
			);
	...
	describe("scanInstalledWorkflows", () => {
		it("should detect installed workflows across tools", async () => {
			...
			await fs.mkdir(path.join(claudeSkillsDir, "openspec-apply-change"), {
				recursive: true,
			});
			await fs.writeFile(
				path.join(claudeSkillsDir, "openspec-apply-change", "SKILL.md"),
				"content",
			);
		...
		it("should return union of workflows across multiple tools", async () => {
			...
			// Cursor has apply
			const cursorSkillsDir = path.join(testDir, ".cursor", "skills");
			await fs.mkdir(path.join(cursorSkillsDir, "openspec-apply-change"), {
				recursive: true,
			});
			await fs.writeFile(
				path.join(cursorSkillsDir, "openspec-apply-change", "SKILL.md"),
				"content",
			);
		...
		it("should only match workflows in ALL_WORKFLOWS", async () => {
			...
			const skillsDir = path.join(testDir, ".claude", "skills");
			await fs.mkdir(path.join(skillsDir, "my-custom-skill"), {
				recursive: true,
			});
			await fs.writeFile(
	...
	describe("tools output", () => {
		it("should list affected tools in output", async () => {
			...
			const hasToolsList = calls.some(
				(call) => call.includes("Tools:") && call.includes("Claude Code"),
			);
