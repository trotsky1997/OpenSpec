...
describe("config command integration", () => {
	...
	beforeEach(() => {
		...
		tempDir = path.join(
			os.tmpdir(),
			`openspec-config-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it("should preserve unknown fields", async () => {
		const { getGlobalConfig, getGlobalConfigDir } = await import(
			"../../src/core/global-config.js"
		);
...
describe("config key validation", () => {
	it("rejects unknown top-level keys", async () => {
		const { validateConfigKeyPath } = await import(
	...
	it("rejects deeply nested feature flag keys", async () => {
		...
		expect(validateConfigKeyPath("featureFlags.someFlag.extra").valid).toBe(
			false,
		);
	});
...
describe("config profile command", () => {
	...
	beforeEach(() => {
		tempDir = path.join(
			os.tmpdir(),
			`openspec-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		fs.mkdirSync(tempDir, { recursive: true });
		originalEnv = { ...process.env };
		process.env.XDG_CONFIG_HOME = tempDir;
	...
	it("custom workflow selection should set profile to custom", async () => {
		...
		const selectedWorkflows = ["explore", "new", "apply", "ff", "clarify"];
		const isCoreMatch =
	...
	it("config schema should validate profile and delivery values", async () => {
		...
		expect(
			validateConfig({ featureFlags: {}, profile: "core", delivery: "both" })
				.success,
		).toBe(true);
		expect(
			validateConfig({
