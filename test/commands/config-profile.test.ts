	...
	it("uses explicit labels when workflows are added and removed", async () => {
		...
		const diff = diffProfileState(
			{ profile: "custom", delivery: "both", workflows: ["propose", "sync"] },
			{
				profile: "custom",
				delivery: "both",
				workflows: ["propose", "clarify"],
			},
		);

		expect(diff.hasChanges).toBe(true);
		expect(diff.lines).toEqual(["workflows: added clarify; removed sync"]);
	});
});

describe("deriveProfileFromWorkflowSelection", () => {
	it("returns custom for an empty workflow selection", async () => {
		const { deriveProfileFromWorkflowSelection } = await import(
			"../../src/commands/config.js"
		);
...
describe("config profile interactive flow", () => {
	...
	function setupDriftedProjectArtifacts(projectDir: string): void {
		...
		const exploreSkillPath = path.join(
			projectDir,
			".claude",
			"skills",
			"openspec-explore",
	...
	function setupSyncedCoreBothArtifacts(projectDir: string): void {
		...
		for (const commandId of coreCommands) {
			for (const namespace of ["opsx", "ssx"]) {
				...
				fs.writeFileSync(commandPath, `# ${namespace}:${commandId}\n`, "utf-8");
			}
		}
	}

	function addExtraSyncWorkflowArtifacts(projectDir: string): void {
		const syncSkillPath = path.join(
			projectDir,
			".claude",
			"skills",
			"openspec-sync-specs",
		...
		for (const namespace of ["opsx", "ssx"]) {
			const syncCommandPath = path.join(
				projectDir,
				".claude",
				"commands",
				namespace,
			...
			fs.writeFileSync(syncCommandPath, `# ${namespace}:sync\n`, "utf-8");
		}
	}
	...
	beforeEach(() => {
		...
		tempDir = path.join(
			os.tmpdir(),
			`openspec-config-profile-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		fs.mkdirSync(tempDir, { recursive: true });

		originalEnv = { ...process.env };
		originalCwd = process.cwd();
		originalTTY = (process.stdout as NodeJS.WriteStream & { isTTY?: boolean })
			.isTTY;
	...
	it("workflows-only action should not invoke delivery prompt", async () => {
		...
		const { PUBLIC_WORKFLOWS } = await import("../../src/core/profiles.js");
		const { select, checkbox } = await getPromptMocks();
		...
		expect(checkboxCall.pageSize).toBe(PUBLIC_WORKFLOWS.length);
		...
		const proposeChoice = checkboxCall.choices.find(
			(choice: { value: string }) => choice.value === "propose",
		);
		const onboardChoice = checkboxCall.choices.find(
			(choice: { value: string }) => choice.value === "onboard",
		);
	...
	it("workflow picker should use friendly names with descriptions", async () => {
		...
		expect(checkboxCall.choices).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
					value: "propose",
					name: "Propose change",
				...
				expect.objectContaining({
					...
					description:
						"Resolve ambiguous scope, requirements, or next-step decisions interactively",
				}),
			]),
		);
	});
	...
	it("selecting current values only should be a no-op and should not ask apply", async () => {
		const { saveGlobalConfig, getGlobalConfigPath } = await import(
			"../../src/core/global-config.js"
		);
	...
	it("keep action should warn when project files drift from global config", async () => {
		...
		expect(consoleLogSpy).toHaveBeenCalledWith(
			expect.stringContaining(
				"Warning: Global config is not applied to this project.",
			),
		);
	});
	...
	it("keep action should not warn when project files are already synced", async () => {
		...
		const allLogs = consoleLogSpy.mock.calls.map((args) =>
			args.map(String).join(" "),
		);
		expect(
			allLogs.some((line) =>
				line.includes("Warning: Global config is not applied to this project."),
			),
		).toBe(false);
	});
