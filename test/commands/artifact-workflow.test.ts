...
describe("artifact-workflow CLI commands", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "openspec-artifact-workflow-"),
		);
	...
	afterEach(async () => {
		if (tempDir) {
			...
			const tempPrefix = `${path.basename(tempDir)}-`;
	...
	async function createTestChange(
		...
		if (artifacts.includes("specs")) {
			...
			await fs.writeFile(
				path.join(specsDir, "test-spec.md"),
				"## Purpose\nTest spec.",
			);
		}
	...
	describe("status command", () => {
		it("shows status for scaffolded change without proposal.md", async () => {
			...
			const result = await runCLI(["status", "--change", "scaffolded-change"], {
				cwd: tempDir,
			});
		...
		it("shows status for a change with proposal and design", async () => {
			...
			const result = await runCLI(["status", "--change", "partial-change"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(0);
		...
		it("outputs JSON when --json flag is used", async () => {
			...
			const result = await runCLI(
				["status", "--change", "json-change", "--json"],
				{
					cwd: tempDir,
				},
			);
			...
			const proposalArtifact = json.artifacts.find(
				(a: any) => a.id === "proposal",
			);
		...
		it("shows complete status when all artifacts are done", async () => {
			...
			const result = await runCLI(["status", "--change", "complete-change"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(0);
		...
		it("errors for unknown change name and lists available changes", async () => {
			...
			const result = await runCLI(["status", "--change", "nonexistent"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
		...
		it("errors for unknown schema", async () => {
			...
			const result = await runCLI(
				["status", "--change", "test-change", "--schema", "unknown"],
				{
					cwd: tempDir,
				},
			);
		...
		it("rejects path traversal in change name", async () => {
			const result = await runCLI(["status", "--change", "../foo"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
		...
		it("rejects absolute path in change name", async () => {
			const result = await runCLI(["status", "--change", "/etc/passwd"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
		...
		it("rejects slashes in change name", async () => {
			const result = await runCLI(["status", "--change", "foo/bar"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
	...
	describe("instructions command", () => {
		it("shows instructions for proposal on scaffolded change", async () => {
			...
			const result = await runCLI(
				["instructions", "proposal", "--change", "scaffolded-change"],
				{
					cwd: tempDir,
				},
			);
		...
		it("shows instructions for design artifact", async () => {
			...
			const result = await runCLI(
				["instructions", "design", "--change", "instr-change"],
				{
					cwd: tempDir,
				},
			);
		...
		it("shows blocked warning for artifact with unmet dependencies", async () => {
			...
			const result = await runCLI(
				["instructions", "tasks", "--change", "blocked-change"],
				{
					cwd: tempDir,
				},
			);
		...
		it("outputs JSON for instructions", async () => {
			...
			const result = await runCLI(
				["instructions", "design", "--change", "json-instr", "--json"],
				{
					cwd: tempDir,
				},
			);
		...
		it("errors when artifact argument is missing", async () => {
			...
			const result = await runCLI(["instructions", "--change", "test-change"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
		...
		it("errors for unknown artifact", async () => {
			...
			const result = await runCLI(
				["instructions", "unknown-artifact", "--change", "test-change"],
				{
					cwd: tempDir,
				},
			);
	...
	describe("templates command", () => {
		...
		it("errors for unknown schema", async () => {
			const result = await runCLI(["templates", "--schema", "nonexistent"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
	...
	describe("new change command", () => {
		it("creates a new change directory", async () => {
			const result = await runCLI(["new", "change", "my-new-feature"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(0);
			const output = getOutput(result);
		...
		it("errors for invalid change name with spaces", async () => {
			const result = await runCLI(["new", "change", "invalid name"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
		...
		it("errors for duplicate change name", async () => {
			...
			const result = await runCLI(["new", "change", "existing-change"], {
				cwd: tempDir,
			});
			expect(result.exitCode).toBe(1);
			const output = getOutput(result);
	...
	describe("instructions apply command", () => {
		it("shows apply instructions for spec-driven schema with tasks", async () => {
			...
			const result = await runCLI(
				["instructions", "apply", "--change", "apply-change"],
				{
					cwd: tempDir,
				},
			);
		...
		it("shows blocked state when required artifacts are missing", async () => {
			...
			const result = await runCLI(
				["instructions", "apply", "--change", "blocked-apply"],
				{
					cwd: tempDir,
				},
			);
		...
		it("outputs JSON for apply instructions", async () => {
			...
			const json = JSON.parse(result.stdout);
			const expectedProposalPath = canonical(
				path.join(changesDir, "json-apply", "proposal.md"),
			);
			const expectedSpecPath = canonical(
...
`,
			...
			const statusResult = await runCLI(
				["status", "--change", "single-star-glob", "--json"],
				{
					cwd: tempDir,
				},
			);
		...
		it("shows schema instruction from apply block", async () => {
			...
			const result = await runCLI(
				["instructions", "apply", "--change", "instr-apply"],
				{
					cwd: tempDir,
				},
			);
		...
		it("shows all_done state when all tasks are complete", async () => {
			...
			const result = await runCLI(
				["instructions", "apply", "--change", "done-apply"],
				{
					cwd: tempDir,
				},
			);
		...
		it("fallback: requires all artifacts when schema has no apply block", async () => {
			...
			const noApplySchemaDir = path.join(
				userDataDir,
				"openspec",
				"schemas",
				"no-apply",
...
`;
			await fs.writeFile(
				path.join(noApplySchemaDir, "schema.yaml"),
				schemaContent,
			);
