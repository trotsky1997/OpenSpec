...
- **THEN** create 10 public skill directories under `.<tool>/skills/`:
...
- **AND** it SHALL not generate a public `openspec-inspect-change/SKILL.md`
  artifact
...
- **WHEN** a tool is selected during initialization
- **THEN** create 10 public slash command files using the tool's command
  adapter:
  - `/opsx:explore`
  - `/opsx:new`
  - `/opsx:continue`
  ...
  - `/opsx:archive`
  - `/opsx:bulk-archive`
- **AND** it SHALL not generate a public `/opsx:inspect` command
