...
Every `SolidSpec`-managed code file SHALL map to one canonical shadow
`impl-spec` path, including test files.

#### Scenario: Source file receives a shadow impl-spec

- **WHEN** a source file is added to a `SolidSpec` change
...
- **WHEN** a test file is added to a `SolidSpec` change
...
- **WHEN** the system resolves shadow paths for a managed file in a `SolidSpec`
  change
...
- **WHEN** a `SolidSpec` change passes its completion verification for a managed
  file
