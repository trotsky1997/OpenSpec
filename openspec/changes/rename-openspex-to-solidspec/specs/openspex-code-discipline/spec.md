...
Every `SolidSpec` change SHALL declare an explicit code-discipline manifest
before formal implementation can proceed.

#### Scenario: Declare discipline profile for a change

- **WHEN** a `SolidSpec` change is prepared for implementation
...
- **WHEN** a discipline manifest is defined for a `SolidSpec` change
...
- **WHEN** a `SolidSpec`-managed file implements important state, boundaries, or
  domain concepts
...
Declared validation commands SHALL be mandatory acceptance gates for
`SolidSpec` changes.

#### Scenario: Validation command failure blocks acceptance

- **WHEN** a required validation command fails for a `SolidSpec` change
...
- **WHEN** all required validation commands pass for a `SolidSpec` change
